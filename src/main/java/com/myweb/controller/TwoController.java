...
public class TwoController {
    ...
    @ApiOperation(value = "我的借书/还书申请", notes = "我的借书申请列表，列表状态为2时可以进行开始借书操作，状态为5时可以进行支付押金操作，状态为7时可进行申请还书操作，状态为10时可进行拒绝扣押金操作或还书成功操作")
    @ApiImplicitParams({
            @ApiImplicitParam(name = "userid", value = "申请者（必需）", required = true, dataType = "Integer"),
            @ApiImplicitParam(name = "status", value = "申请状态（可选，默认为0），0全部，1发起申请，2已同意，3已拒绝，4开始借书（不要押金），5待付押金，6,已付押金，7已确认（已经借出）,8申请还书，9还书确认，10扣除押金，11拒绝扣押金，12还书成功", required = true, dataType = "Integer")
    })
    @ResponseBody
